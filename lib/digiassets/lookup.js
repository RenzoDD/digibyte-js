...
AssetLookup.prototype.setProvider = function (url, token = "digibyte-rocks") {
    this._provider = url;
    this._token = token;
    return this;
}
AssetLookup.prototype.getToken = async function (url, utxo) {
    var data = await get(url, { utxo });
    if (!data)
...
AssetLookup.prototype._getUrl = async function (key) {
    if (this._accessKeyId && this._accessKeyId)
        return this._signAWS(key);
    else if (this.provider) {
        var data = await get(this._provider + key, { token: this._token });
        if (!data)
