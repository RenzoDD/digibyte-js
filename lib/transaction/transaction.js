...
Transaction.prototype.checkedSerialize = function(opts) {
  ...
  if (serializationError) {
    serializationError.message += ' - For more information please see: ' +
      'https://github.com/RenzoDD/digibyte-js/blob/master/docs/transaction.md#serialization-checks';
 ...
 *
 * Can receive, as output information, the output of digibyted's `listunspent` command,
 * and a slightly fancier format recognized:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a Address object. The
 * same is true for `script`, which can be a string or a Script.
 *
...
Transaction.prototype.from = function(utxo, pubkeys, threshold, opts) {
  ...
  if (utxo.address && !(utxo.script || utxo.scriptPubKey))
    utxo.scriptPubKey = Script.fromAddress(utxo.address).toHex();
  if (!utxo.satoshis && utxo.amount)
    utxo.satoshis = Math.round(parseFloat(utxo.amount) * 1e8);
  if (!utxo.satoshis && utxo.value)
    utxo.satoshis = parseInt(utxo.amount);
 ...
 * CheckTransaction in digibyte core.
 */
