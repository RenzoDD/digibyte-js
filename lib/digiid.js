...
DigiID.prototype._path = function (index = 0) {
    ...
    var A = hash.slice(0, 4).readUInt32LE() % 0x80000000;
    var B = hash.slice(4, 8).readUInt32LE() % 0x80000000;
    var C = hash.slice(8, 12).readUInt32LE() % 0x80000000;
    var D = hash.slice(12, 16).readUInt32LE() % 0x80000000;

    return `m/13'/${A}'/${B}'/${C}'/${D}'`;
}
DigiID.prototype.sign = function (secret, index = 0, address, assetAddress, assetKey) {
    ...
    if (!hdPrivateKey && !privateKey) throw 'Invalid secret';
    if (hdPrivateKey) {
        var path = this._path(index);
        var privateKey = hdPrivateKey.deriveChild(path).privateKey;
    }

    if (!Address.isValid(address)) address = privateKey.toAddress().toString();
    var signature = new Message(this.uri).sign(privateKey);
    var result = { uri: this.uri, address, signature };

    if (this.asset.dual) {
        if (!Address.isValid(assetAddress)) throw "Invalid asset address";
        if (!PrivateKey.isValid(assetKey)) throw "Invalid asset private key";
...
DigiID.prototype.login = async function (secret, index = 0, address, assetAddress, assetKey) {
    try {
        var data = await fetch(this.callback, {
            method: "POST",
            body: JSON.stringify(this.sign(secret, index = 0, address, assetAddress, assetKey)),
