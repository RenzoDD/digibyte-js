...
function Address(data, network, type, multisigType) {
  ...
  type = Address.FriendlyType(type);
...
Address.FriendlyType = function (value) {
  switch (value) {
    case 'legacy': case 'p2pkh': case 'pkh':
      return Address.PayToPublicKeyHash;
    case 'script': case 'p2sh': case 'sh':
      return Address.PayToScriptHash;
    case 'segwit': case 'p2wpkh': case 'wpkh':
      return Address.PayToWitnessPublicKeyHash;
    case 'compatibility': case 'p2wsh': case 'wsh':
...
Address._transformHash = function (hash, network, type) {
  ...
  info.type = Address.FriendlyType(type);
  return info;
};
...
/**
 * Internal function to transform a digibyte address buffer
 *
 * @param {Buffer} buffer - An instance of a hex encoded address Buffer
 ...
 *
 * The addresses will be sorted lexicographically, as that is the trend in digibyte.
...
/**
 * Internal function to transform a digibyte address string
 *
 * @param {string} data
 * @param {String|Network=} network - either a Network instance, 'livenet', or 'testnet'
 * @param {string=} type - The type: 'pubkeyhash', 'scripthash', 'witnesspubkeyhash', or 'witnessscripthash'
 ...
 *
 * @param {string} str - An string of the digibyte address
 * @param {String|Network=} network - either a Network instance, 'livenet', or 'testnet'
 * @param {string=} type - The type of address: 'script' or 'pubkey'
 * @returns {Address} A new valid and frozen instance of an Address
 */
 ...
 *
 * @returns {Buffer} DigiByte address buffer
 */
 ...
 *
 * @returns {string} DigiByte address
 */
