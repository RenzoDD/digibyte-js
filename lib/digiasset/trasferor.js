...
Transferor.prototype = {
    get assetsIn() {
        ...
        if (Object.keys(assets) > 1)
            throw "This library can't handle different asset inputs";
        return amountIn;
    },
    get assetsOut() {
        var amountOut = 0;
    ...
    get assetsBurned() {
        ...
        var deflation = 0;
        if (this._rules.deflation)
            deflation = this.isConsolidation ? 0 : this._rules.deflation;
        return this._toBurn + deflation;
    },
    get assetsChange() {
        if (this._burnExtra)
            return 0;
        return this.assetsIn - this.assetsOut - this.assetsBurned;
    },

    get changes() {
        var changes = {};
        for (var input of this._inputs)
            changes[input.address] = (changes[input.address] || 0) - input.assets[0].count;

        for (var output of this._outputs)
            changes[output.address] = (changes[output.address] || 0) + output.count;

        this._rules = { rules: this._rules };
        var change = this.assetsChange;
        this._rules = this._rules.rules;
        
        if (change > 0) {
            if (!Address.isValid(this._assetChange))
                throw "Define change address or burn extra assets";
            changes[this._assetChange] = (changes[this._assetChange] || 0) + change; 
        }

        return changes;
    },
    get isConsolidation() {
        for (var change of Object.values(this.changes))
    ...
    get outputs() {
        ...
        if (!this.isConsolidation)
            if ((this._rules.voting || {}).restricted)
                for (var address of Object.keys(this.changes)) {
                    if (this.changes[address] <= 0) continue;
                    if (!Vote.isValidAddress(address)) throw "This asset can't be send to a non voting address";
                }

        if (this.assetsChange > 0) {
            if (Address.isValid(this._assetChange))
                result.push({ address: this._assetChange, satoshis: 600 });
            else throw "Define change address or burn extra assets";
    ...
    get royalty() {
        ...
        if (this._rules.royalty === undefined)
            return [];

        var exchange = 100000000;
        if (this._rules.royalty.units !== undefined) {
            if (!this._price) throw "Must provide a price object";

            if (typeof this._rules.royalty.units == "string" && this._rules.royalty.units.name != "DGB")
                this._rules.royalty.units = { ...Price.publishers.find(x => x.name === this._rules.royalty.units) };

            if (typeof this._rules.royalty.units == "object")
                exchange = Math.round((this._price.find(x => x.address === this._rules.royalty.units.address && x.index === this._rules.royalty.units.index) || {}).value);
        }
        ...
        for (var address of addresses) {
            var satoshis = Price.Multiply(this._rules.royalty.addresses[address], exchange);
...
Transferor.prototype.addInputs = function (utxos) {
    if (typeof utxos.length == 'undefined') utxos = [utxos];

    for (var utxo of utxos) {
        if (!utxo.address) utxo.address = Address.fromScript(utxo.script || utxo.scriptPubKey).toString();
        if (utxo.assets) {
            if (utxo.assets.length == 1) this._inputs.push(utxo);
            else if (utxo.assets.length == 0) this._gas.push(utxo);
            else throw "This library can't handle multi-asset inputs";
        }
