...
Transferor.prototype = {
    ...
    get assetsBurned() {
        var deflation = this.isConsolidation ? 0 : Number(this._rules.deflation || 0);

        if (this._burnExtra) {
            var remaining = this.assetsIn - this.assetsOut;
            return this._toBurn + remaining + deflation;
        }
    ...
    get changes() {
        ...
        if (this.assetsChange > 0) {
            if (Address.isValid(this._assetChange))
                changes[this._assetChange] = (changes[this._assetChange] || 0) + this.assetsChange;
    ...
    get royalty() {
        ...
        if (this._rules.royalty.units !== undefined) {
            ...
            if (typeof this._rules.royalty.units == "string" && this._rules.royalty.units.name != "DGB")
                this._rules.royalty.units = Price.objects.find(x => x.name === this._rules.royalty.units);
