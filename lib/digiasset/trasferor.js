...
Transferor.prototype = {
    ...
    get instructions() {
        ...
        for (var output of this._outputs) {
            data.addInteger(0, 3);
            data.addInteger(n++, 5);
            data.addPrecision(output.quantity);
        }
...
Transferor.prototype.addInputs = function (utxos) {
    ...
    for (var utxo of utxos) {
        if (utxo.assetId) this._inputs.push(utxo);
 ...
 * @param {int} amount
 */
Transferor.prototype.addOutput = function (address, quantity) {
    this._outputs.push({ address, quantity });
    return this;
}
