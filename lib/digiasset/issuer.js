...
Issuer.prototype = {
    ...
    get priceOutput() {
        ...
        if (Price.publishers.findIndex(x => x.address === this._rules.royalty.units.address) === -1)
    ...
    get instructions() {
        ...
        if (this._rules.isEmpty()) data.addHex("01");
...
Issuer.prototype.setStorage = function (bytes, price) {
    var exchange = NaN;
    if (typeof price === 'number') exchange =  price;
    else if (price.USD) exchange = price.USD;
    else if (price.value) exchange = price.value;
    else if (price.length) {
        var publisher = price.find(x => x.address === Price.addresses.fiat && x.index === 1);
        if (publisher) exchange = publisher.value;
        var publisher = price.find(x => x.name === 'USD');
        if (publisher) exchange = publisher.value;
    }
