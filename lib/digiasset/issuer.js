...
Issuer.prototype = {
    ...
    get instructions() {
        ...
        if (this._rules !== false) {
            ...
            if (this._rules.vote || this._rules.expiry) {
                if (this._rules.changeable === true)
                    throw "Votes can not be changeable";

                // expiry is really vote with no options so can't do both
                if (this._rules.vote && this._rules.expiry)
                    throw "Invalid Rule Detected: can't use both vote and expiry";

                if (this._rules.expiry)
                    this._rules.vote = { options: [], movable: true, cutoff: BigInt(this._rules.expiry) };
