...
function Metadata(data) {
    if (data) this.data = (typeof data == "string") ? JSON.parse(data) : data;
    else this.data = {};
}
 ...
 * @returns 
 */
Metadata.prototype.vote = function (vote) {
    if (this.votes)
        delete this.votes;

    this.votes = [];
    for (var n in vote.options)
        this.votes.push({ address: Vote.addresses[n], label: vote.options[n] });
