 ...
 *
 * @param {Address|String} digibyteAddress - A DigiByte address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(digibyteAddress, signatureString) {
  $.checkArgument(digibyteAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(digibyteAddress)) {
    digibyteAddress = Address.fromString(digibyteAddress);
  }
  ...
  var signatureAddress = Address.fromPublicKey(publicKey, digibyteAddress.network);

  // check that the recovered address and specified address match
  if (digibyteAddress.toString() !== signatureAddress.toString()) {
 ...
 * @returns {String}
 */
Message.prototype.recoverPublicKey = function recoverPublicKey(digibyteAddress, signatureString) {
  $.checkArgument(digibyteAddress);
  $.checkArgument(signatureString && _.isString(signatureString));
